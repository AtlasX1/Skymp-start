 ...
 */
export function getPos(formId: number): [number, number, number] {
	// const [x, y, z]: number[] = mp.get(formId, 'pos');
	return mp.get(formId, 'pos');
}
 ...
 * @param formId ID Object
 */
export function getAngle(formId: number): [number, number, number] {
	// const [x, y, z]: number[] = mp.get(formId, 'angle');
	return mp.get(formId, 'angle');
}
